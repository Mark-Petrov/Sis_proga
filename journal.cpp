...
void Journal::append(const std::string &path, std::uint64_t ts_us,
...
std::vector<JournalRecord> Journal::read(const std::string &path) {
    ...
    while (pos < data.size()) {
        ...
        std::string line = nl == std::string::npos ? data.substr(pos)
...
std::uint64_t parse_ts_string(const std::string &s, std::string &err) {
    ...
    int n = std::sscanf(s.c_str(), "%d.%d.%d-%d:%d:%d.%d", &y, &mo, &d, &h, &mi,
                        &se, &ms);
    if (n < 6) {
    ...
    if (n >= 7)
        base += static_cast<std::uint64_t>(ms % 1000000);
    return base;
}
