...
void Parser::expect(TokKind k, const char *ctx) {
    if (cur_.kind != k)
        throw ParseError{"parse: unexpected token in " + std::string(ctx)};
    advance();
}

void Parser::expect_kw(const std::string &kw) {
    if (cur_.kind != TokKind::Kw || cur_.text != kw)
        throw ParseError{"expected keyword " + kw};
    advance();
}
...
int Parser::parse_predicate(WhereClause &w) {
    ...
    if (check_text("LIKE")) {
        advance();
        if (!check(TokKind::String))
            throw ParseError{"LIKE expects string pattern"};
        LikeExpr L;
        L.col = col;
        L.pattern = cur_.text;
        advance();
...
Statement Parser::parse_one() {
    if (cur_.kind != TokKind::Kw)
        throw ParseError{"statement must start with keyword"};
    ...
    if (k == "CREATE") {
        ...
        if (check_text("USER")) {
            ...
            expect_kw("PASSWORD");
            Literal lit = parse_literal();
            if (lit.kind != Literal::String)
                throw ParseError{"password must be string"};
            u.password = lit.str;
            return u;
        }
        if (check_text("GROUP")) {
    ...
    if (k == "REVERT") {
        ...
        if (cur_.kind != TokKind::Ident)
            throw ParseError{"expected timestamp"};
        r.ts = cur_.text;
        advance();
        return r;
    }
