...
static BPlusTreeIndex::Cmp make_cmp(const StringPool *pool) {
    return [pool](const Value &a, const Value &b) {
        return value_cmp(a, b, *pool);
    };
}

Table::Table(std::string name, std::vector<ColumnDef> columns,
             SharedStringPool pool)
    ...
    for (std::size_t i = 0; i < cols_.size(); ++i) {
        if (cols_[i].indexed) {
            indexes_[i] =
                std::make_unique<BPlusTreeIndex>(make_cmp(pool_.get()), 32);
        }
    }
}
...
bool Table::validate_row(const Row &r, std::string &err) const {
    ...
    for (int i = 0; i < static_cast<int>(cols_.size()); ++i) {
        ...
        if (!v.is_null()) {
            ...
            if (cols_[i].type == ColType::String &&
...
BPlusTreeIndex *Table::index_for_column(int col_idx) {
    if (col_idx < 0 || col_idx >= static_cast<int>(indexes_.size()))
        return nullptr;
