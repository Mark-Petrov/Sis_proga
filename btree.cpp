...
BPlusTreeIndex::BPlusTreeIndex(Cmp cmp, int order)
    : cmp_(std::move(cmp)), order_(std::max(4, order)) {}
...
BPlusTreeIndex::Node *BPlusTreeIndex::find_leaf(Node *r, const Value &k) const {
    ...
    while (cur && !cur->leaf) {
        int i = 0;
        while (i < static_cast<int>(cur->keys.size()) &&
               cmpk(cur->keys[i], k) <= 0)
            ++i;
...
void BPlusTreeIndex::insert_leaf(Node *leaf, const Value &key, RowId row) {
    int pos = 0;
    while (pos < static_cast<int>(leaf->keys.size()) &&
           cmpk(leaf->keys[pos], key) < 0)
        ++pos;
...
void BPlusTreeIndex::insert_value(const Value &key, RowId row) {
    ...
    auto insert_sep = [&](BPlusTreeIndex::Node *parent, const Value &sep,
                          BPlusTreeIndex::Node *left,
                          BPlusTreeIndex::Node *right) {
        int idx = 0;
        while (idx < static_cast<int>(parent->ch.size()) &&
               parent->ch[idx] != left)
            ++idx;
    ...
    while (cur && static_cast<int>(cur->keys.size()) > MAXK) {
        ...
        int pidx = 0;
        while (pidx < static_cast<int>(cur->parent->ch.size()) &&
               cur->parent->ch[pidx] != cur)
            ++pidx;
...
bool BPlusTreeIndex::erase(const Value &key, RowId row) {
    ...
    for (Node *l = leaf; l; l = l->next) {
        if (erase_leaf(l, key, row))
            return true;
    }
    return false;
}

std::vector<BPlusTreeIndex::RowId>
BPlusTreeIndex::find_equal(const Value &key) const {
...
std::vector<BPlusTreeIndex::RowId> BPlusTreeIndex::range(const Value &low,
                                                         const Value &high,
                                                         bool low_incl,
