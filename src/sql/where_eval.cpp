...
static const Value* cell_by_name(const Table& table, const Row& row, const std::string& name) {
  int idx = table.column_index(name);
  if (idx < 0) return nullptr;
  return &row.cells[static_cast<std::size_t>(idx)];
}

static bool cmp_values(sql::CmpOp op, const Value& a, const Value& b, const StringPool& pool) {
...
static bool eval_node(const sql::WhereClause& w, int idx, const Table& table, const Row& row,
...
static bool eval_node(const sql::WhereClause& w, int idx, const Table& table, const Row& row,
  ...
  switch (n.kind) {
    ...
    case sql::WhereNodeKind::AndOr: {
      ...
      if (!err.empty()) return false;
      if (n.bin.kind == sql::BinBool::And) return L && R;
      return L || R;
    }
    ...
    case sql::WhereNodeKind::Between: {
      ...
      return cmp_values(sql::CmpOp::Ge, *cv, a, pool) && cmp_values(sql::CmpOp::Le, *cv, b, pool);
    }
