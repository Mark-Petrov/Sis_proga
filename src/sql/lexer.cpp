...
Token Lexer::next() {
  ...
  int c = col_;
  char ch = peek();
  if (ch == '\0') return Token{TokKind::Eof, "", l, c};
  ...
  if (ch == '"') {
    ...
    while (true) {
      char x = get();
      if (x == '\0') return Token{TokKind::String, s, l, c};
      if (x == '"') break;
      s.push_back(x);
    }
  ...
  if (std::isdigit(static_cast<unsigned char>(ch))) {
    ...
    int sc = col_;
    std::string buf;
    while (std::isdigit(static_cast<unsigned char>(peek())) || peek() == '.' || peek() == '-' ||
           peek() == ':') {
      buf.push_back(get());
    }
  ...
  if (std::isalpha(static_cast<unsigned char>(ch)) || ch == '_') {
    ...
    static const std::unordered_set<std::string> kws = {
        "CREATE", "DATABASE", "DROP", "USE", "TABLE", "INT",    "STRING", "NOT",    "NULL",
        "INDEXED", "INSERT",  "INTO", "VALUE", "VALUES", "UPDATE", "SET",    "WHERE",  "DELETE",
        "FROM",    "SELECT",  "AS",   "BETWEEN", "AND",   "OR",     "LIKE",   "REVERT", "DEFAULT",
        "SUM",     "COUNT",     "AVG",  "LOGIN",  "PASSWORD", "USER", "GROUP", "GRANT", "TO", "ON"};
    std::string up = to_upper(id);
    if (kws.count(up)) return Token{TokKind::Kw, up, l, c};
