...
bool Database::create_table(const std::string& tname, std::vector<ColumnDef> cols,
                            SharedStringPool pool, std::string& err) {
  if (tables_.count(tname)) {
    err = "table already exists: " + tname;
    return false;
  }
  tables_[tname] = std::make_shared<Table>(tname, std::move(cols), std::move(pool));
  return true;
}
